...
from expenses.infrastructure.mappers.expense_mapper import ExpenseMapper
from expenses.infrastructure.repositories.schema import Expense


class ExpenseRepository(WriteRepository, ReadRepository):
    ...
    async def get_by_id(self, aggregateId: UniqueEntityId) -> Either[
        ...
        query = await self.db.scalars(
            select(Expense).where(Expense.id == aggregateId.value)
        )
    ...
    async def exists(self, aggregateId: UniqueEntityId) -> bool:
        query = await self.db.scalars(
            select(exists().where(Expense.id == aggregateId.value))
        )
